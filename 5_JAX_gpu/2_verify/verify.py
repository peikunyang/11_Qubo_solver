...
jax_result_root = solve_root / "solution"

output_root = Path("solution")
summary_file = Path("verify_jax_energy_summary.csv")
...
def read_jax_energy(filename):
    if not filename.is_file():
        return None

    with open(filename, "r") as f:
        for line in f:
            if "JAX Energy" in line or "Energy" in line:
...
if not jax_result_root.is_dir():
    raise NotADirectoryError(f"solution folder not found: {jax_result_root}")
    ...
    for threshold_path in sorted(data_folder_path.iterdir(), key=sort_key):
        ...
        for data_id_path in sorted(threshold_path.iterdir(), key=sort_key):
            ...
            for solution_file in sorted(data_id_path.iterdir(), key=sort_key):
                ...
                if not solution_file.name.startswith("jax_"):
                    continue

                seed = solution_file.name.replace("jax_", "", 1)
                ...
                output_file = output_folder / f"pytorch_verify_jax_{seed}"
                ...
                try:
                    ...
                    jax_energy = read_jax_energy(jax_result_file)

                    if jax_energy is None:
                        diff = "NA"
                        status = "JAX_ENERGY_NOT_FOUND"
                    else:
                        diff_value = pytorch_energy - jax_energy
                        diff = diff_value

                        if is_match(pytorch_energy, jax_energy):
                            status = "MATCH"
                        else:
                            status = "DIFF"
                    ...
                    with open(output_file, "w") as f:
                        ...
                        if jax_energy is not None:
                            f.write(f"JAX Reported Energy: {jax_energy}\n")
                            f.write(f"Energy Difference: {pytorch_energy - jax_energy}\n")
                            f.write(f"Status: {status}\n")
                        else:
                            f.write("JAX Reported Energy: NA\n")
                    ...
                    rows.append([
                        ...
                        seed,
                        pytorch_energy,
                        jax_energy if jax_energy is not None else "NA",
                        diff,
                        str(jax_result_file),
                        status
                    ])
