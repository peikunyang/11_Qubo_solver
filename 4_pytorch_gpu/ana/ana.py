...
Time=[]
Step=[]
datas=['1_data_1000','2_data_6000','3_data_25000','4_data_45000']

for i in range (1,7,1):
  for data in datas:
    for j in range (1,6,1):
      for k in range (1,6,1):
        filename = f"../{i}_th_-{i}/{data}/1_solve/solution/{j}/pytorch_{k}"
...
with open("4_pytorchg_ene", "w") as file:
...
with open("4_pytorchg_time", "w") as file:
  for i in range(len_data):
    file.write('%s\n'%datas[i])
    for m in range(6):
      file.write('%8.3f\n' % Timen[m][i]) 
...
with open("4_pytorchg_step", "w") as file:
  for i in range(len_data):
    file.write('%s\n'%datas[i])
    for m in range(6):
      file.write('%7d\n' % Stepn[m][i])
