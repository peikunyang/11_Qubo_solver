...
pytorch_result_root = solve_root / "solution"

output_root = Path("solution")
summary_file = Path("verify_pytorch_energy_summary.csv")
...
def read_pytorch_energy(filename):
    if not filename.is_file():
        return None

    with open(filename, "r") as f:
        for line in f:
            if "PyTorch Energy" in line or "Pytorch Energy" in line or "Energy" in line:
...
if not pytorch_result_root.is_dir():
    raise NotADirectoryError(f"solution folder not found: {pytorch_result_root}")
    ...
    for threshold_path in sorted(data_folder_path.iterdir(), key=sort_key):
        ...
        for data_id_path in sorted(threshold_path.iterdir(), key=sort_key):
            ...
            for solution_file in sorted(data_id_path.iterdir(), key=sort_key):
                ...
                if not solution_file.name.startswith("pytorch_"):
                    continue

                seed = solution_file.name.replace("pytorch_", "", 1)
                ...
                output_file = output_folder / f"pytorch_verify_{seed}"
                ...
                try:
                    ...
                    reported_energy = read_pytorch_energy(pytorch_result_file)

                    if reported_energy is None:
                        diff = "NA"
                        status = "PYTORCH_ENERGY_NOT_FOUND"
                    else:
                    ...
                    with open(output_file, "w") as f:
                        ...
                        if reported_energy is not None:
                            f.write(f"PyTorch Reported Energy: {reported_energy}\n")
                            f.write(f"Energy Difference: {verified_energy - reported_energy}\n")
                            f.write(f"Status: {status}\n")
                        else:
                            f.write("PyTorch Reported Energy: NA\n")
