...
def read_scipy_results(filename):
    energy = None
    time_used = None
    step = None
    ...
    for data in datas:
        for j in range(1, num_data_id + 1):
            ...
            for k in range(1, num_seed + 1):
                ...
                energy, time_used, step = read_scipy_results(filename)
...
with open("6_SciPy_ene", "w") as file:
...
with open("6_SciPy_time", "w") as file:
    for i in range(len_data):
        file.write("%s\n" % datas[i])
...
with open("6_SciPy_step", "w") as file:
    for i in range(len_data):
        file.write("%s\n" % datas[i])
