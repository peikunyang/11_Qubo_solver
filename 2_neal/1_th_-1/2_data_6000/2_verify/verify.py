...
def load_Q(filename):
    ...
    return torch.tensor(Q_np, dtype=torch.float32, device='cpu')
...
Q = load_Q(f"../../../../1_gen_coe/2_data_{Num}/data_{i}.npy")
solution = load_solution(f"../1_solve/solutionX/{i}/pyqubo_{j}")
computed_energy = verify_solution(Q, solution)

with open(f"solution/{i}/pyqubo_{j}", "w") as f:
